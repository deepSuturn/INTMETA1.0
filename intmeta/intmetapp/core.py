...
def kraken(fileinput, taxonomiclevel, fileinput2=None):
    ...
    output_df = df_data2[['Info5', 'Info1']].copy()

    # Substituição dos cabecalhos para melhor visualizacao no D3.JS
    # Segue a logica (Original:Saida_D3JS)

    output_df = output_df.rename(columns={'Info1': 'data', 'Info5': 'name'}, inplace=False)
    print(output_df)
    # Pega apenas os 10 com maior numero de reads.
    output_df_out = output_df.nlargest(20, 'data')
    ...
    output_df_2 = output_df_2.nlargest(9, columns=['Info1'])
    output_df_2 = output_df_2.reset_index()
    output_df_2 = output_df_2.drop('index', axis=1)
    # Pega nome do organismo mais classificado
    most_classified_organism = output_df_2.Info5[0]
    ...
    output_df_2 = output_df_2.rename(columns={'Info1': 'data', 'Info5': 'name'}, inplace=False)
    # -----------------------------------------------------------------

    # -----------------------------------------------------------------
    # Retira maior porcentagem para ser parametro de tamanho do gráfico
    maxreads = output_df_2['data'].iloc[0]
    # -----------------------------------------------------------------
    ...
    dfd3_2 = output_df_2.to_dict('r')
    return dfd3, dfd3_2, maxpercent, maxreads, total_reads, most_classified_organism
...
def centrifuge(fileinput, taxonomiclevel):
    ...
    df_data_reads = df_data[['Info0', 'Info4']].copy()
    df_data_reads = df_data_reads.rename(columns={'Info4': 'Info2', 'Info0': 'Info1'}, inplace=False)
